...
class Model(base.Model):
    ...
    def setup_optimizer(self, opt):
        ...
        if opt.nerf.fine_sampling:
            self.optim.add_param_group(
                dict(params=self.graph.nerf_fine.parameters(),
                     lr=opt.optim.lr))
        # set up scheduler
        if opt.optim.sched:
            ...
            if opt.optim.lr_end:
                ...
                opt.optim.sched.gamma = (opt.optim.lr_end /
                                         opt.optim.lr)**(1. / opt.max_iter)
    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        if split == "train":
            ...
            if opt.nerf.fine_sampling:
                ...
                self.tb.add_scalar("{0}/{1}".format(split, "lr_fine"), lr,
                                   step)
        # compute PSNR
        ...
        if opt.nerf.fine_sampling:
            ...
            self.tb.add_scalar("{0}/{1}".format(split, "PSNR_fine"), psnr,
                               step)
    ...
    def visualize(self, opt, var, step=0, split="train", eps=1e-10):
        if opt.tb:
            ...
            if not opt.nerf.rand_rays or split != "train":
                invdepth = (
                    1 - var.depth) / var.opacity if opt.camera.ndc else 1 / (
                        var.depth / var.opacity + eps)
                rgb_map = var.rgb.view(-1, opt.H, opt.W,
                                       3).permute(0, 3, 1, 2)  # [B,3,H,W]
                invdepth_map = invdepth.view(-1, opt.H, opt.W,
                                             1).permute(0, 3, 1,
                                                        2)  # [B,1,H,W]
                util_vis.tb_image(opt, self.tb, step, split, "rgb", rgb_map)
                util_vis.tb_image(opt, self.tb, step, split, "invdepth",
                                  invdepth_map)
                if opt.nerf.fine_sampling:
                    invdepth = (
                        1 - var.depth_fine
                    ) / var.opacity_fine if opt.camera.ndc else 1 / (
                    ...
                    invdepth_map = invdepth.view(-1, opt.H, opt.W,
                                                 1).permute(0, 3, 1,
                                                            2)  # [B,1,H,W]
                    util_vis.tb_image(opt, self.tb, step, split, "rgb_fine",
                                      rgb_map)
                    util_vis.tb_image(opt, self.tb, step, split,
                                      "invdepth_fine", invdepth_map)
    ...
    def evaluate_full(self, opt, eps=1e-10):
        ...
        for i, batch in enumerate(loader):
            ...
            invdepth = (1 -
                        var.depth) / var.opacity if opt.camera.ndc else 1 / (
                            var.depth / var.opacity + eps)
            rgb_map = var.rgb.view(-1, opt.H, opt.W, 3).permute(0, 3, 1,
                                                                2)  # [B,3,H,W]
            invdepth_map = invdepth.view(-1, opt.H, opt.W,
    ...
    def generate_videos_synthesis(self, opt, eps=1e-10):
        ...
        if opt.data.dataset == "blender":
            ...
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/rgb_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1"
                .format(test_path, rgb_vid_fname))
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/depth_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1"
                .format(test_path, depth_vid_fname))
        else:
            ...
            idx_center = (poses -
                          poses.mean(dim=0, keepdim=True))[..., 3].norm(
                              dim=-1).argmin()
            pose_novel = camera.get_novel_view_poses(opt,
                                                     poses[idx_center],
                                                     N=60,
                                                     scale=scale).to(
                                                         opt.device)
            ...
            pose_novel_tqdm = tqdm.tqdm(pose_novel,
                                        desc="rendering novel views",
                                        leave=False)
            intr = edict(next(iter(self.test_loader))).intr[:1].to(
                opt.device)  # grab intrinsics
            for i, pose in enumerate(pose_novel_tqdm):
                ...
                invdepth = (
                    1 - ret.depth) / ret.opacity if opt.camera.ndc else 1 / (
                        ret.depth / ret.opacity + eps)
                rgb_map = ret.rgb.view(-1, opt.H, opt.W,
                                       3).permute(0, 3, 1, 2)  # [B,3,H,W]
                invdepth_map = invdepth.view(-1, opt.H, opt.W,
                                             1).permute(0, 3, 1,
                                                        2)  # [B,1,H,W]
            ...
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/rgb_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1"
                .format(novel_path, rgb_vid_fname))
            os.system(
                "ffmpeg -y -framerate 30 -i {0}/depth_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1"
                .format(novel_path, depth_vid_fname))
...
class Graph(base.Graph):
    ...
    def forward(self, opt, var, mode=None):
        ...
        if opt.nerf.rand_rays and mode in ["train", "test-optim"]:
            ...
            var.ray_idx = torch.randperm(
                opt.H * opt.W,
                device=opt.device)[:opt.nerf.rand_rays // batch_size]
            ret = self.render(opt,
                              pose,
                              intr=var.intr,
                              ray_idx=var.ray_idx,
    ...
    def render(self, opt, pose, intr=None, ray_idx=None, mode=None):
        batch_size = len(pose)
        center, ray = camera.get_center_and_ray(opt, pose,
                                                intr=intr)  # [B,HW,3]
        while ray.isnan().any(
        ...
        rgb_samples, density_samples = self.nerf.forward_samples(opt,
                                                                 center,
                                                                 ray,
                                                                 depth_samples,
                                                                 mode=mode)
        ...
        if opt.nerf.fine_sampling:
            with torch.no_grad():
                ...
                depth_samples = torch.cat([depth_samples, depth_samples_fine],
                                          dim=2)  # [B,HW,N+Nf,1]
    ...
    def sample_depth(self, opt, batch_size, num_rays=None):
        ...
        rand_samples = torch.rand(
            batch_size, num_rays, opt.nerf.sample_intvs, 1,
    ...
    def sample_depth_from_pdf(self, opt, pdf):
        ...
        grid = torch.linspace(0,
                              1,
                              opt.nerf.sample_intvs_fine + 1,
                              device=opt.device)  # [Nf+1]
        unif = 0.5 * (grid[:-1] + grid[1:]).repeat(*cdf.shape[:-1],
                                                   1)  # [B,HW,Nf]
        idx = torch.searchsorted(cdf, unif,
                                 right=True)  # [B,HW,Nf] \in {1...N}
        # inverse transform sampling from CDF
        depth_bin = torch.linspace(depth_min,
                                   depth_max,
                                   opt.nerf.sample_intvs + 1,
                                   device=opt.device)  # [N+1]
        depth_bin = depth_bin.repeat(*cdf.shape[:-1], 1)  # [B,HW,N+1]
        depth_low = depth_bin.gather(dim=2,
                                     index=(idx - 1).clamp(min=0))  # [B,HW,Nf]
        depth_high = depth_bin.gather(
            dim=2, index=idx.clamp(max=opt.nerf.sample_intvs))  # [B,HW,Nf]
...
class NeRF(torch.nn.Module):
    ...
    def define_network(self, opt):
        ...
        for li, (k_in, k_out) in enumerate(L):
            ...
            if opt.arch.tf_init:
                self.tensorflow_init_weights(opt,
                                             linear,
                                             out="first" if li == len(L) -
                                             1 else None)
    ...
    def forward(self, opt, points_3D, ray_unit=None, mode=None):  # [B,...,3]
        if opt.arch.posenc:
            points_enc = self.positional_encoding(opt,
                                                  points_3D,
                                                  L=opt.arch.posenc.L_3D)
            points_enc = torch.cat([points_3D, points_enc],
                                   dim=-1)  # [B,...,6L+3]
        else:
        ...
        for li, layer in enumerate(self.mlp_feat):
            ...
            if li == len(self.mlp_feat) - 1:
                ...
                if opt.nerf.density_noise_reg and mode == "train":
                    density += torch.randn_like(
                        density) * opt.nerf.density_noise_reg
                density_activ = getattr(
                    torch_F,
                    opt.arch.density_activ)  # relu_,abs_,sigmoid_,exp_....
        ...
        if opt.nerf.view_dep:
            ...
            if opt.arch.posenc:
                ray_enc = self.positional_encoding(opt,
                                                   ray_unit,
    ...
    def forward_samples(self, opt, center, ray, depth_samples, mode=None):
        ...
        rgb_samples, density_samples = self.forward(
            opt, points_3D_samples, ray_unit=ray_unit_samples,
