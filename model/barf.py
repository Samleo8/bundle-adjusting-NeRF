...
class Model(nerf.Model):
    ...
    def build_networks(self, opt):
        ...
        if opt.camera.noise:
            ...
            se3_noise = torch.randn(len(self.train_data), 6,
                                    device=opt.device) * opt.camera.noise
            self.graph.pose_noise = camera.lie.se3_to_SE3(se3_noise)
        self.graph.se3_refine = torch.nn.Embedding(len(self.train_data),
                                                   6).to(opt.device)
    ...
    def setup_optimizer(self, opt):
        ...
        self.optim_pose = optimizer([
            dict(params=self.graph.se3_refine.parameters(),
                 lr=opt.optim.lr_pose)
        ])
        # set up scheduler
        if opt.optim.sched_pose:
            scheduler = getattr(torch.optim.lr_scheduler,
                                opt.optim.sched_pose.type)
            if opt.optim.lr_pose_end:
                assert (opt.optim.sched_pose.type == "ExponentialLR")
                opt.optim.sched_pose.gamma = (
                    opt.optim.lr_pose_end / opt.optim.lr_pose)**(1. /
                                                                 opt.max_iter)
            kwargs = {
                k: v
                for k, v in opt.optim.sched_pose.items() if k != "type"
            }
    ...
    def train_iteration(self, opt, var, loader):
        ...
        if opt.optim.warmup_pose:
            ...
            self.optim_pose.param_groups[0][
                "lr_orig"] = self.optim_pose.param_groups[0][
        ...
        if opt.optim.warmup_pose:
            self.optim_pose.param_groups[0][
                "lr"] = self.optim_pose.param_groups[0][
                    "lr_orig"]  # reset learning rate
    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        if split == "train" and opt.data.dataset in ["blender", "llff"]:
            ...
            self.tb.add_scalar("{0}/error_R".format(split), error.R.mean(),
                               step)
            self.tb.add_scalar("{0}/error_t".format(split), error.t.mean(),
                               step)
    ...
    def prealign_cameras(self, opt, pose, pose_GT):
        ...
        try:
            ...
            sim3 = edict(t0=0,
                         ...
                         s0=1,
                         s1=1,
                         R=torch.eye(3, device=opt.device))
        # align the camera poses
        center_aligned = (center_pred -
                          sim3.t1) / sim3.s1 @ sim3.R.t() * sim3.s0 + sim3.t0
    ...
    def evaluate_test_time_photometric_optim(self, opt, var):
        ...
        iterator = tqdm.trange(opt.optim.test_iter,
    ...
    def generate_videos_pose(self, opt):
        ...
        os.system(
            "ffmpeg -y -r 30 -f concat -i {0} -pix_fmt yuv420p {1} >/dev/null 2>&1"
            .format(list_fname, cam_vid_fname))
...
class Graph(nerf.Graph):
    ...
    # TODO: Where is the reconstruction loss (implicit in the original function except pose is now a variable??)
    ...
    def get_pose(self, opt, var, mode=None):
        if mode == "train":
            ...
            center_aligned = (center -
                              sim3.t0) / sim3.s0 @ sim3.R * sim3.s1 + sim3.t1
...
class NeRF(nerf.NeRF):
    ...
    def __init__(self, opt):
        ...
        self.progress = torch.nn.Parameter(
            torch.tensor(0.))  # use Parameter so it could be checkpointed
    ...
    def positional_encoding(self, opt, input, L):  # [B,...,N]
        ...
        if opt.barf_c2f is not None:
            ...
            weight = (1 -
                      (alpha - k).clamp_(min=0, max=1).mul_(np.pi).cos_()) / 2
            # apply weights
